...
void main() {
  ...
  StateMachine buildSelfAndAncestor() {
    log = [];
    ActionFn logAs(String tag) => (c, e) => log.add(tag);
    return StateMachine.fromJson(selfAndAncestorChart, actions: {
      'enterP': logAs('enterP'),
      'exitP': logAs('exitP'),
  ...
  test('leaf self-transition: entry+exit actions both fire, state stays active', () {
  ...
  test('child targeting its own ancestor re-enters through the initial chain', () {
    final m = buildSelfAndAncestor();
    expect(m.matches('p.c1'), isTrue);
    log.clear();
  ...
  test(
      'region-root target inside a parallel machine restarts that region at '
      'its initial child; other regions are untouched', () {
    final log = <String>[];
    final m = StateMachine.fromJson(parallelRegionRootChart, actions: {
